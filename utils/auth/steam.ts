...
export async function getSteamAuthenticationURL(): Promise<string> {
  ...
  return await new Promise<string>((resolve, reject) => {
    steamRelyingParty.authenticate(
...
export async function getSteamUserIdentifier(
  request: NextRequest
): Promise<string> {
  const steamRelyingParty = getSteamRelyingParty();
    ...
    steamRelyingParty.verifyAssertion(request.url, (error, result) => {
      ...
      if (
        !searchParams
          ...
          ?.startsWith(OPENID_CHECK.claimed_id)
      ) {
        reject(new Error("Invalid OpenID claimed identity."));
      }

      if (
        !searchParams.get("openid.identity")?.startsWith(OPENID_CHECK.identity)
      ) {
